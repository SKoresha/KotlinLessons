...
object KotlinWorkshop3 {
    ...
    private fun playRound(numb1: Int, numb2: Int): Boolean {
        when {
            numb1 == numb2 -> {
                println("Congratulations!")
                return true
            }
            numb1 > numb2 -> println("Your Guess is higher, continue.")
            numb1 < numb2 -> println("Your Guess is lower, continue.")
        }
        return false
    }
    ...
    private fun printTotalCountOfGuesses(guessCounter: Int) {
        println("Общее число попыток ввода $guessCounter")
    }
    ...
    private fun countHigherGuesses(
        ...
        var counter = 0
        guesses.forEach { if (it>randomNumber) counter++ }
        printer (counter)
        return counter
    }
    ...
    val countLowerGuesses: (IntArray, Int) -> Unit = { guesses, randomNumber ->
        var counter = 0
        guesses.forEach { if (it<randomNumber && it ==-1) counter++ }
        println("Total count of higher guesses: $counter\n")
    }
